...
try:
    from seahub.settings import ENABLE_OFFICE_WEB_APP, OFFICE_SERVER_TYPE
except ImportError:
...
#        'obj_id': file_id,
    ...
    if filetype in (TEXT, MARKDOWN) or fileext in get_conf_text_ext():
        ...
        if ENABLE_OFFICE_WEB_APP and OFFICE_SERVER_TYPE != 'collaboraoffice':
            print 'a'
            action_name = None
            ...
            if ENABLE_OFFICE_WEB_APP_EDIT and permission == 'rw' and \
                ...
                return_dict['can_edit_file'] = True
            ...
            if wopi_dict:
                ...
                send_file_access_msg(request, repo, path, 'web')
                return_dict.update(wopi_dict)
                return render(request, 'view_file_wopi.html', return_dict)
        ...
        if ENABLE_ONLYOFFICE:
            ...
            if permission == 'rw' and ((not is_locked) or (is_locked and locked_by_me)) and \
            ...
            return_dict.update(onlyoffice_dict)
            return render(request, 'view_file_onlyoffice.html', return_dict)
...
@login_required
@repo_passwd_set_required
def wopi_file_edit(request, repo_id):
    ...
    if permission != 'rw' or repo.encrypted or \
            not ENABLE_OFFICE_WEB_APP_EDIT or \
            fileext not in OFFICE_WEB_APP_EDIT_FILE_EXTENSION or \
    ...
    wopi_dict = get_wopi_dict(username, repo_id, path,
            'edit', request.LANGUAGE_CODE)

    if wopi_dict:
        wopi_dict['doc_title'] = doc_title
        send_file_access_msg(request, repo, path, 'web')
        return render(request, 'edit_file_wopi.html', wopi_dict)
    else:
        logger.debug(wopi_dict)
        return render_error(request, _(u'Unable to edit file'))

@login_required
@repo_passwd_set_required
def onlyoffice_file_edit(request, repo_id):
    ...
    if permission != 'rw' or repo.encrypted or \
            not ENABLE_ONLYOFFICE or \
            fileext not in ONLYOFFICE_EDIT_FILE_EXTENSION or \
            (is_locked and not locked_by_me):
        return render_permission_error(request, _(u'Unable to edit file'))
    ...
    send_file_access_msg(request, repo, path, 'web')
    return render(request, 'onlyoffice/edit_file_onlyoffice.html', {
        'ONLYOFFICE_APIJS_URL': ONLYOFFICE_APIJS_URL,
        'file_type': fileext,
        'doc_key': doc_key,
        'doc_title': filename,
